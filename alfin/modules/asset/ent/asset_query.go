...
// QueryPkg chains the current query on the "pkg" edge.
func (aq *AssetQuery) QueryPkg() *WorkloadPkgQuery {
	query := &WorkloadPkgQuery{config: aq.config}
...
// WithPkg tells the query-builder to eager-load the nodes that are connected to
// the "pkg" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AssetQuery) WithPkg(opts ...func(*WorkloadPkgQuery)) *AssetQuery {
	query := &WorkloadPkgQuery{config: aq.config}
...
func (aq *AssetQuery) sqlAll(ctx context.Context) ([]*Asset, error) {
	...
	if query := aq.withPkg; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Asset)
		for i := range nodes {
			if nodes[i].workload_pkg_assets == nil {
				continue
			}
			fk := *nodes[i].workload_pkg_assets
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(workloadpkg.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "workload_pkg_assets" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Pkg = n
			}
