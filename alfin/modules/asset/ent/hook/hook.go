...
// The WorkloadPkgFunc type is an adapter to allow the use of ordinary
// function as WorkloadPkg mutator.
type WorkloadPkgFunc func(context.Context, *ent.WorkloadPkgMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WorkloadPkgFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.WorkloadPkgMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WorkloadPkgMutation", m)
	}
	return f(ctx, mv)
}
