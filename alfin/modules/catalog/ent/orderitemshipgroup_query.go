...
func (oisgq *OrderItemShipGroupQuery) QueryOrderAdjustments() *OrderAdjustmentQuery {
	query := &OrderAdjustmentQuery{config: oisgq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oisgq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oisgq *OrderItemShipGroupQuery) QueryOrderItemShipGrpInvRes() *OrderItemShipGrpInvResQuery {
	query := &OrderItemShipGrpInvResQuery{config: oisgq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oisgq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oisgq *OrderItemShipGroupQuery) QueryOrderPaymentPreferences() *OrderPaymentPreferenceQuery {
	query := &OrderPaymentPreferenceQuery{config: oisgq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oisgq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oisgq *OrderItemShipGroupQuery) Clone() *OrderItemShipGroupQuery {
	...
	return &OrderItemShipGroupQuery{
		...
		withOrderAdjustments:         oisgq.withOrderAdjustments.Clone(),
		withOrderItemShipGroupAssocs: oisgq.withOrderItemShipGroupAssocs.Clone(),
		withOrderItemShipGrpInvRes:   oisgq.withOrderItemShipGrpInvRes.Clone(),
		withOrderPaymentPreferences:  oisgq.withOrderPaymentPreferences.Clone(),
...
func (oisgq *OrderItemShipGroupQuery) WithOrderAdjustments(opts ...func(*OrderAdjustmentQuery)) *OrderItemShipGroupQuery {
	query := &OrderAdjustmentQuery{config: oisgq.config}
	for _, opt := range opts {
		opt(query)
	}
	oisgq.withOrderAdjustments = query
	return oisgq
}
...
func (oisgq *OrderItemShipGroupQuery) WithOrderItemShipGrpInvRes(opts ...func(*OrderItemShipGrpInvResQuery)) *OrderItemShipGroupQuery {
	query := &OrderItemShipGrpInvResQuery{config: oisgq.config}
	for _, opt := range opts {
		opt(query)
	}
	oisgq.withOrderItemShipGrpInvRes = query
	return oisgq
}
...
func (oisgq *OrderItemShipGroupQuery) WithOrderPaymentPreferences(opts ...func(*OrderPaymentPreferenceQuery)) *OrderItemShipGroupQuery {
	query := &OrderPaymentPreferenceQuery{config: oisgq.config}
	for _, opt := range opts {
		opt(query)
	}
	oisgq.withOrderPaymentPreferences = query
	return oisgq
}
...
func (oisgq *OrderItemShipGroupQuery) sqlAll(ctx context.Context) ([]*OrderItemShipGroup, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			oisgq.withOrderAdjustments != nil,
			oisgq.withOrderItemShipGroupAssocs != nil,
			oisgq.withOrderItemShipGrpInvRes != nil,
			oisgq.withOrderPaymentPreferences != nil,
		}
	)
	...
	if query := oisgq.withOrderAdjustments; query != nil {
		...
		query.Where(predicate.OrderAdjustment(func(s *sql.Selector) {
			s.Where(sql.InValues(orderitemshipgroup.OrderAdjustmentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.order_item_ship_group_order_adjustments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "order_item_ship_group_order_adjustments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "order_item_ship_group_order_adjustments" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := oisgq.withOrderItemShipGrpInvRes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*OrderItemShipGroup)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.OrderItemShipGrpInvRes(func(s *sql.Selector) {
			s.Where(sql.InValues(orderitemshipgroup.OrderItemShipGrpInvResColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.order_item_ship_group_order_item_ship_grp_inv_res
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "order_item_ship_group_order_item_ship_grp_inv_res" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "order_item_ship_group_order_item_ship_grp_inv_res" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OrderItemShipGrpInvRes = append(node.Edges.OrderItemShipGrpInvRes, n)
		}
	}

	if query := oisgq.withOrderPaymentPreferences; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*OrderItemShipGroup)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.OrderPaymentPreference(func(s *sql.Selector) {
			s.Where(sql.InValues(orderitemshipgroup.OrderPaymentPreferencesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.order_item_ship_group_order_payment_preferences
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "order_item_ship_group_order_payment_preferences" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "order_item_ship_group_order_payment_preferences" returned %v for node %v`, *fk, n.ID)
			}
...
func (oisggb *OrderItemShipGroupGroupBy) sqlQuery() *sql.Selector {
	selector := oisggb.sql.Select()
	aggregation := make([]string, 0, len(oisggb.fns))
	...
	return selector.GroupBy(selector.Columns(oisggb.fields...)...)
}
