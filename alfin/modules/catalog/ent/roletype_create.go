...
func (rtc *RoleTypeCreate) AddOrderRoleIDs(ids ...int) *RoleTypeCreate {
	rtc.mutation.AddOrderRoleIDs(ids...)
	return rtc
}

// AddOrderRoles adds the "order_roles" edges to the OrderRole entity.
func (rtc *RoleTypeCreate) AddOrderRoles(o ...*OrderRole) *RoleTypeCreate {
	ids := make([]int, len(o))
	for i := range o {
		ids[i] = o[i].ID
	}
	return rtc.AddOrderRoleIDs(ids...)
}
...
func (rtc *RoleTypeCreate) createSpec() (*RoleType, *sqlgraph.CreateSpec) {
	...
	if nodes := rtc.mutation.OrderRolesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
