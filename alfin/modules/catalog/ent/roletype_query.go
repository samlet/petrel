...
func (rtq *RoleTypeQuery) QueryOrderRoles() *OrderRoleQuery {
	query := &OrderRoleQuery{config: rtq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rtq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rtq *RoleTypeQuery) Clone() *RoleTypeQuery {
	...
	return &RoleTypeQuery{
		...
		withOrderRoles:     rtq.withOrderRoles.Clone(),
...
func (rtq *RoleTypeQuery) WithOrderRoles(opts ...func(*OrderRoleQuery)) *RoleTypeQuery {
	query := &OrderRoleQuery{config: rtq.config}
	for _, opt := range opts {
		opt(query)
	}
	rtq.withOrderRoles = query
	return rtq
}
...
func (rtq *RoleTypeQuery) sqlAll(ctx context.Context) ([]*RoleType, error) {
	...
	if query := rtq.withOrderRoles; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*RoleType)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.OrderRole(func(s *sql.Selector) {
			s.Where(sql.InValues(roletype.OrderRolesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.role_type_order_roles
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "role_type_order_roles" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "role_type_order_roles" returned %v for node %v`, *fk, n.ID)
			}
