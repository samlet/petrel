...
type StatusValidChange struct {
	...
	order_item_status_valid_changes       *int
...
func (*StatusValidChange) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case statusvalidchange.ForeignKeys[0]: // order_item_status_valid_changes
			values[i] = new(sql.NullInt64)
		case statusvalidchange.ForeignKeys[1]: // status_item_main_status_valid_changes
			values[i] = new(sql.NullInt64)
		case statusvalidchange.ForeignKeys[2]: // status_item_to_status_valid_changes
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case statusvalidchange.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field order_item_status_valid_changes", value)
			} else if value.Valid {
				svc.order_item_status_valid_changes = new(int)
				*svc.order_item_status_valid_changes = int(value.Int64)
			}
		...
		case statusvalidchange.ForeignKeys[2]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
