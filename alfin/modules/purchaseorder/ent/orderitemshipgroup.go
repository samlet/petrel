...
// was not loaded in eager-loading.
func (e OrderItemShipGroupEdges) PrimaryShipmentsOrErr() ([]*Shipment, error) {
	if e.loadedTypes[2] {
...
func (*OrderItemShipGroup) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case orderitemshipgroup.FieldTrackingNumber, orderitemshipgroup.FieldShippingInstructions, orderitemshipgroup.FieldMaySplit, orderitemshipgroup.FieldGiftMessage, orderitemshipgroup.FieldIsGift:
...
// QueryPrimaryShipments queries the "primary_shipments" edge of the OrderItemShipGroup entity.
func (oisg *OrderItemShipGroup) QueryPrimaryShipments() *ShipmentQuery {
	return (&OrderItemShipGroupClient{config: oisg.config}).QueryPrimaryShipments(oisg)
}
