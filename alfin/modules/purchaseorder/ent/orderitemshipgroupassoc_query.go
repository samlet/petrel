...
func (oisgaq *OrderItemShipGroupAssocQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(orderitemshipgroupassoc.Columns...)...).From(t1)
	if oisgaq.sql != nil {
		selector = oisgaq.sql
		selector.Select(selector.Columns(orderitemshipgroupassoc.Columns...)...)
	}
...
func (oisgagb *OrderItemShipGroupAssocGroupBy) sqlQuery() *sql.Selector {
	selector := oisgagb.sql
	columns := make([]string, 0, len(oisgagb.fields)+len(oisgagb.fns))
	columns = append(columns, oisgagb.fields...)
	for _, fn := range oisgagb.fns {
		columns = append(columns, fn(selector))
	}
	return selector.Select(columns...).GroupBy(oisgagb.fields...)
}
...
func (oisgas *OrderItemShipGroupAssocSelect) sqlScan(ctx context.Context, v interface{}) error {
	rows := &sql.Rows{}
	query, args := oisgas.sqlQuery().Query()
...
func (oisgas *OrderItemShipGroupAssocSelect) sqlQuery() sql.Querier {
	selector := oisgas.sql
	selector.Select(selector.Columns(oisgas.fields...)...)
	return selector
}
