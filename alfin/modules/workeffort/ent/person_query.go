...
func (pq *PersonQuery) QueryParty() *PartyQuery {
	query := &PartyQuery{config: pq.config}
...
func (pq *PersonQuery) QueryUserLogins() *UserLoginQuery {
...
func (pq *PersonQuery) WithParty(opts ...func(*PartyQuery)) *PersonQuery {
	query := &PartyQuery{config: pq.config}
...
func (pq *PersonQuery) WithUserLogins(opts ...func(*UserLoginQuery)) *PersonQuery {
	query := &UserLoginQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PersonQuery) sqlAll(ctx context.Context) ([]*Person, error) {
	...
	if query := pq.withParty; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Person)
		for i := range nodes {
			if nodes[i].party_person == nil {
				continue
			}
			fk := *nodes[i].party_person
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "party_person" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withUserLogins; query != nil {
		...
		query.Where(predicate.UserLogin(func(s *sql.Selector) {
			s.Where(sql.InValues(person.UserLoginsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.person_user_logins
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "person_user_logins" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "person_user_logins" returned %v for node %v`, *fk, n.ID)
			}
