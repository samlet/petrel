...
func (tx *Tx) init() {
	tx.Workload = NewWorkloadClient(tx.config)
	tx.WorkloadFeature = NewWorkloadFeatureClient(tx.config)
	tx.WorkloadFeatureAppl = NewWorkloadFeatureApplClient(tx.config)
	tx.WorkloadFeatureApplType = NewWorkloadFeatureApplTypeClient(tx.config)
	tx.WorkloadItem = NewWorkloadItemClient(tx.config)
	tx.WorkloadStatus = NewWorkloadStatusClient(tx.config)
	tx.WorkloadType = NewWorkloadTypeClient(tx.config)
}
...
// applies a query, for example: Workload.QueryXXX(), the query will be executed
