...
func genConsts(pkgInfo *build.Package, typeNames *string) {
	consts := getConsts(pkgInfo, typeNames)
	src := genString(pkgInfo, consts)
...
func getConsts(pkgInfo *build.Package, typeNames *string) map[string][]string {
	//获得待处理的Type
	types := strings.Split(*typeNames, ",")
...
func genString(pkgInfo *build.Package, types map[string][]string) []byte {
	const strTmp = `
	package {{.pkg}}
	import "fmt"
