...
/**
$ just wire wiretut run hi
$ just wire wiretut service hi
*/
...
type CLI struct {
	...
	Service ServiceCmd `cmd help:"Run a service"`
}
...
func main() {
	...
	ctx := kong.Parse(&cli,
		kong.Name("wiretut"),
		kong.Description("A wiretut cli"),
		kong.UsageOnError(),
...
func (cmd *ServiceCmd) Run(globals *Globals) error {
...
// need to run at first time: $ wire
